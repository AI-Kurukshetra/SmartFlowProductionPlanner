-- ============================================
-- Core Modules: Plant Management, Products, BOM
-- Run in Supabase SQL Editor
-- ============================================

-- Ensure current_user_organization_id exists (from migration 004)
...
AS $$
  SELECT organization_id FROM public.app_users WHERE id = auth.uid() LIMIT 1;
$$;

-- ============================================
-- 1. PLANT MANAGEMENT - Resources
-- ============================================

-- work_centers (id, plant_id, name) - ensure exists
...
-- resources - manufacturing resources with capacity
...
CREATE POLICY "Org members manage resources" ON public.resources FOR ALL
  USING (
    work_center_id IN (
      SELECT wc.id FROM public.work_centers wc
